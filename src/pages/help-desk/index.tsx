...
export const HelpDeskPage = () => {
  ...
  const userOnboardingGuide: OnboardingSection[] = [
    ...
    },
    {
      ...
      description:
        "Enable Admins to manage and personalize their own account as well as handle user-level access within the application. This includes adding new users, updating user information, managing credentials, and handling account activation or deactivation.",
      content: [
        {
  ...
  return (
    <div className={clsx("pb-10", appUser ? "space-y-1" : "space-y-10")}>
      <div className="bg-primary-300 shadow-xl p-3 flex flex-col justify-end items-center">
        <h6 className="text-2xl font-normal text-center">How do we help?</h6>

        {!appUser && (
          <div className="mt-5">
            <AppButton onClick={() => navigate("/")}>Login Now</AppButton>
          </div>
        )}
      </div>
      <div className="grid grid-cols-12 gap-5 p-3 sm:p-5 md:p-10">
        <div className="space-y-5 col-span-12">
          <h1
            className={clsx(
              "text-xl sm:text-2xl capitalize pb-4 border-b",
              appUser ? "mb-2" : " mb-4 "
            )}
          >
            Help Topic
          </h1>
          <div className="xl:grid-cols-2 grid gap-x-5 sm:gap-x-10 gap-y-5 grid-cols-1">
            {userOnboardingGuide.map(
              ({ Icon, description, title, ...props }, i) => (
                <div
                  onClick={() => {
                    setDetails({
                      Icon,
                  ...
                  key={i}
                  className="flex items-start sm:items-center gap-3 sm:gap-5 group cursor-pointer border-b pb-5"
                >
                  <div className="flex items-center text-primary-500 gap-3">
                    <Icon className="size-8 sm:size-10" />
                  </div>
                  <div className="flex-1">
                    <p className="capitalize text-base sm:text-lg group-hover:text-primary-500">
                      {title}
                    </p>
      ...
      <div className="py-5 mx-3 sm:mx-5 flex flex-col sm:flex-row items-center justify-between bg-gray-200 rounded-md p-3 gap-3 sm:gap-0">
        <div className="sm:flex-1">
          <h5 className="text-sm sm:text-md">Need Support</h5>
      ...
      {details && (
        ...
        >
          <div className="flex gap-3 sm:gap-5 items-center">
            <details.Icon className="size-8 sm:size-12 rounded-lg text-white bg-primary-500 p-2" />
            <h5 className="text-xl sm:text-2xl font-semibold">
              {details.title}
            </h5>
          </div>
          <p className="font-normal text-xs sm:text-sm text-gray-500 mt-2">
            {details.description}
          </p>
          <div className="h-[50vh] sm:h-[60vh] overflow-y-scroll p-3 my-2">
            {details.content.map(
              ({ details, title }, index) => (
                <div key={index}>
                  ...
                  <div className="my-3 sm:my-6 space-y-2">
                    {details.map(
                      (detail, index) => (
                        <div
                          key={index}
                        ...
                        >
                          {!detail.includes("?") &&
                            !detail.includes(":") && (
                              <div>
                                <IoIosArrowRoundForward className="size-5 sm:size-6 mt-0.5" />
                              </div>
                            )}
