...
export const NewDemoPage = () => {
  ...
  const handleFileChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (demoType === "webgl") {
      dispatch(handleFiles({ ...files, sourceFile: filesArray as File[] })); // Store files array
    } else {
      dispatch(
        handleFiles({
          ...files,
  ...
  const uploadFiles = useCallback(async () => {
    ...
    try {
      const webGlUrls = await Promise.all(
        (files.sourceFile as File[]).map((file) => uploadWebGl(file, "webgl"))
      );
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              ...
              {categoryData && (
                <AppSelect
                  ...
                  defaultValue={""}
                  onChange={(e) => {
                    setFieldValue("product_category_id._id", e.target.value);
                  }}
                  ...
                  value={values.product_category_id._id}
                />
              )}
              ...
              <div className="flex flex-wrap gap-6">
                ...
                {fileUrls.thumbnailUrl && (
                  <div className="flex-1 flex flex-col items-center justify-center pb-5 relative group">
                    <img
                    ...
                    {/* Action buttons */}
                    <div className="absolute top-0 right-0 flex gap-2 p-2 opacity-0 group-hover:opacity-100 transition-opacity">
                      {/* Replace button in its own label */}
                      <label className="cursor-pointer bg-white border border-primary-500 text-primary-500 px-2 py-1 text-xs rounded shadow hover:bg-primary-50 transition-all relative overflow-hidden">
                        Replace
                        <input
                          type="file"
                      ...
                      {/* Remove button is NOT inside label */}
                      <button
                        onClick={() =>
                          dispatch(
                            handleFileUrls({
                        ...
                        className="bg-red-500 text-white px-2 py-1 text-xs rounded shadow hover:bg-red-600 transition-all"
                      >
                        Remove
                      </button>
                    </div>
                ...
                <div>
                  <div className="flex flex-col items-center w-full gap-4 p-6 bg-gray-100 rounded-xl shadow-md animate-pulse">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-lg font-semibold text-blue-600">
                        Your WebGL Files Uploaded Sucessfully!
                      </span>
                    </div>
                  </div>
                </div>
              ...
              <div className="flex justify-end">
                <AppButton
                  disabled={webGlUploading}
                  loading={isCategoryLoading || isLoading}
                  type="submit"
                >
                  Submit
                </AppButton>
