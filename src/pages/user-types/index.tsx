...
export const UserTypesPage = () => {
  ...
  const { filteredUserType, searchInput, newUserTypeModal, newInputs } =
    useUserTypeSlice();
  const { data, isError, error, isLoading, isSuccess } = useGetAllUserTypeQuery(
    { page: 1, size: 10 }
  );
  const [
    NewUserType,
  ...
  const columns: ColumnDef<IUserType>[] = [
    ...
    },
    {
      ...
      cell: ({ row }) => {
        return <div>{moment(row.original.created_at).format("lll")}</div>;
      },
    },
    {
      accessorKey: "_id",
  ...
  return (
    <div>
      ...
      <div className="my-5 flex items-center gap-5">
        ...
        <AppButton onClick={() => dispatch(handleNewUserTypeModal(true))}>
          New User Type
        </AppButton>
      </div>
      {!isLoading && !isDeleteLoading && !isError && filteredUserType && (
        <div>
          <AppTable
      ...
      {isLoading && isDeleteLoading && isCreateLoading && <AppLoader />}
      <AppModal
        width="md"
        action={handleSubmit}
